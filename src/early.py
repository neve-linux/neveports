...
def fire():
    # we need structural pattern matching in templates and nbuild itself
    ...
    if subprocess.run(rcmd, capture_output=True).returncode != 0:
        sys.exit("You have to run nbuild from a git clone")

    # running as root interferes with the sandbox functionality
    if os.geteuid() == 0:
        sys.exit("Please don't run nbuild as root")
