from nbuild.core import chroot, logger, dependencies, profile, scanelf, paths
from nbuild.core import template, update_check as uc, pkg as pkgm, errors
from nbuild.util import flock
from nbuild.apk import cli as apk, generate as apkgen, sign as asign
...
def register_hooks():
    for stepn in hooks:
        dirn = paths.nbuild() / "hooks" / stepn
        if dirn.is_dir():
            for f in dirn.glob("*.py"):
                # this must be skipped
                if f.name == "__init__.py":
                    continue
                modn = "nbuild.hooks." + stepn + "." + f.stem
...
def _restricted_importer(name, globals=None, locals=None, fromlist=(), level=0):
    ...
    if (
        ...
        if name != "nbuild.util":
            raise ImportError(
                f"only modules from nbuild.util allowed in template (got: '{name}')"
            )
...
def invoke_fetch(pkg):
    srclock = paths.sources() / "nbuild.lock"
...
def _build_locked(
    ...
    with (
        ...
        if ret.returncode != 0:
            raise errors.NbuildException("failed to generate packages")
        ...
        for repo in pkg._stage:
            ...
            if not apk.build_index(repo, pkg.source_date_epoch):
                raise errors.NbuildException("indexing repositories failed")
