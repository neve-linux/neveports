...
def keygen(size, eaddr, cfgfile, cfgpath):
    if not shutil.which("openssl"):
        raise errors.NbuildException("openssl is missing")
    ...
    if keypath.is_file():
        raise errors.NbuildException("attempt to overwrite an existing key")
    ...
    if not kout.returncode == 0:
        raise errors.NbuildException("key generation failed")
    ...
    if not pout.returncode == 0:
        raise errors.NbuildException("public key generation failed")
